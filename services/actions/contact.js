...
export const getContactData = (lang) => {
  return (dispatch) => {
    dispatch({ type: SET_CONTACT_LOADED });

    axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/contact-page-text?_locale=${lang}`
      )
      .then((res) => {
        const { data } = res;
...
export const postContact = (formData) => {
  return function (dispatch) {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/question-or-concerns`, formData)
      .then((response) => {
