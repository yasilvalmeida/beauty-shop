...
export const getShopPageText = (lang) => {
  return (dispatch) => {
    dispatch({ type: GET_SHOP_PAGE_TEXT_DATA });
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/shop-page-text?_locale=${lang}`)
      .then((res) => {
        const { data } = res;

        dispatch({
          type: SET_SHOP_PAGE_TEXT_DATA,
          payload: data,
        });
      })
      .catch((err) =>
        dispatch({ type: SET_SHOP_PAGE_TEXT_ERROR, payload: err })
      );
  };
};
