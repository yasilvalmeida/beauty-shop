  ...
  switch (type) {
    ...
    case DELETE_ADDRESSES:
      return {
        ...state,
        billingAddresses:
          payload.appointment === "billing"
            ? state.billingAddresses.filter((item) => item.id !== payload.id)
            : state.billingAddresses,
        deliveryAddresses:
          payload.appointment === "delivery"
            ? state.deliveryAddresses.filter((item) => item.id !== payload.id)
