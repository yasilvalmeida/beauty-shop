...
const VersandartenScene = () => {
  ...
  const versandartenText = useSelector(
    (state) => state.versandarten.versandartenText
  );
  const versandartenLoaded = useSelector(
    (state) => state.versandarten.versandartenTextLoaded
  );
  return (
    <>
      {versandartenLoaded ? (
        ...
        <>
          ...
          <div className={"versandarten__scene__body"}>
            ...
            <TextBoxOne
              headText={versandartenText?.shipping_text}
              header={versandartenText?.header}
              textData={versandartenText?.shipping_conditions}
            />
            <TextBoxTwo
              header={versandartenText?.payment_text}
              leftText={versandartenText?.payment_options}
