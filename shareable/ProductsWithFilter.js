...
const ProductsWithFilter = ({
    ...
    const top3Categories = navList.slice(0, 3);
    const [defaultCategory, setDefaultCategory] = useState(0);
    let category = top3Categories[defaultCategory];
    const productsData = useSelector((state) => state.shop.shopProducts);
    const products = productsData.slice(0, 3);

    useEffect(() => {
      category = top3Categories[defaultCategory];
      dispatch(getShopProducts(1, 3, "KATEGORIEN", category?.id));
    }, [category]);
                ...
                <div className={" first-prod-items col-lg-3"} key={i}>
                  <div className={"picture-body-prod"}>
                    {elem?.images && (
                      <img
                        src={
                          elem.variants_of_a_products.find(
                            (item) => item.main === true
                          ).url
                            ? elem.variants_of_a_products.find(
                                (item) => item.main === true
