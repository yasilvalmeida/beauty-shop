    ...
    return (
        <div className={"cart__sidebar__product__body"}>
            <div className={"cart__sidebar__product__body__image"}>
                <img
                    src={!elem?.variants_of_a_products[0].images[0].url ? elem.images[0].url : elem?.variants_of_a_products[0].images[0].url}
                    alt={""}/>
            </div>
            <div className={"cart__sidebar__product__body__text"}>
                ...
                <span
                    className={"cart__sidebar__product__body__text--price"}>{formatter.format(elem.variants_of_a_products[0].price * value || 0)}</span>
                <div className={"cart__sidebar__product__body__text--quantityinp"}>
                    ...
                    <input type="number" min={"1"} max={"10"} onChange={onChange}
                           value={value < 10 ? `0${value}` : value}/>
                    <button className={"btnminus"} onClick={onDecHandler}><FontAwesomeIcon icon={faMinus}/></button>
                </div>
                <span className={"cart__sidebar__product__body__text--delete"}
                      onClick={() => dispatch(deleteFromBasket(elem?.id, elem.variants_of_a_products[0].id))}>löschen</span>
            </div>
        </div>
    )
}
